...
export const EDITOR_EXTENSIONS_LIST_SHORTCUTS = "type to search • ↑↓ navigate • tab filter • enter toggle • space settings • ctrl+s save+reload • esc back";
...
export class EditorModal<T = string, F extends string = string> implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		const hasDescriptionArea = items.some((item) => item.selectedDescription);
...
export class EditorSettingsModal implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		const template = new EditorDialogTemplate({ theme, size: "compact" });
